...
export async function GET(
  ...
  try {
    ...
    const result = await query('SELECT * FROM drivers WHERE id = $1', [params.id]);
...
export async function PUT(
  ...
  try {
    ...
    const result = await query(
      ...
      [name, email, phone, licenseNumber, status, params.id]
    );
    ...
    console.error('[v0] Update driver error:', error);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}
